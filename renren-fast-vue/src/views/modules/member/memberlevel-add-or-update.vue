<template>
  <el-dialog :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false" :visible.sync="visible">
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="120px">
      <el-form-item label="Lv Name" prop="name">
  ...
  methods: {
    init(id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.id) {
          this.$http({
            url: this.$http.adornUrl(`/member/memberlevel/${this.dataForm.id}`),
            method: 'get',
            params: this.$http.adornParams({ pageSize: 1, pageNum: 1 })
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.dataForm.name = data.data.content[0].name
              this.dataForm.growthPoint = data.data.content[0].growthPoint || 0
              this.dataForm.defaultStatus = data.data.content[0].defaultStatus || 0
              this.dataForm.freeFreightPoint = data.data.content[0].freeFreightPoint || 0
              this.dataForm.commentGrowthPoint = data.data.content[0].commentGrowthPoint || 0
              this.dataForm.priviledgeFreeFreight = data.data.content[0].priviledgeFreeFreight || 0
              this.dataForm.priviledgeMemberPrice = data.data.content[0].priviledgeMemberPrice || 0
              this.dataForm.priviledgeBirthday = data.data.content[0].priviledgeBirthday || 0
              this.dataForm.note = data.data.content[0].note
            }
          })
        }
      })
    ...
    dataFormSubmit() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(`/member/memberlevel`),
            method: 'post',
